...
export const modifyRightistRequest = functions.https.onCall( async (data: RequestModification, context: any) => {
  const {
    email: email = '',
...
export const sendMailApprovedRejectNotificationContribution = functions.database.ref(`/persons/requestArchieve/contributions/{user_id}/{contribution_id}`).onUpdate(async (change, context: any) => {
  const contributorDetails = await fireStore.collection('users').doc(context.params.user_id).get();
